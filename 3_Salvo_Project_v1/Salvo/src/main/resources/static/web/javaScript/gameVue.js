let salvo = new Vue({
            ...
            Promise.all([
                ...
                fetch(url, {
                    ...
                    .then(function (gameJson) {
                        ...
                        salvo.myGameHistory = gameJson.game.myGameHistory
 ...
 //... current player, afterward a function will be use to make sure this page is accessed by the proper person
        ...
        checkBoard(){
            ...
            //needed variable to create a board with boat destroy
                let dom = document.getElementById(checkBoard);
                let p = document.createElement("p");
                let data = [this.myGameHistory];


            //isolating boats with sunk status, it will require three loops as the data have three steps
            for (let i = 0; i < data.length; i++){
               let step1 = [data[i]]
               console.log(data[i])
                for(let j = 0; j < step1[0].length; j++){
                    let b = j + 1;
                    let step2 = step1[0][j]
                    let step0 = ".turnNumber" + b;
                    let step20 = step2+step0;

                    console.log(step2)
                    console.log(step2.length)
                    console.log(step0)
                    console.log(step20)
                    console.log(step20.length)
                    ...
                    for(let k = 0; k < step2.length; k++){
                        ...
                        let step3 = step2[k].shipState
                       
                        if(step3 == "sunk"){
                            
                            p.textContent(step2[k])
                            dom.appendChild(p)
                        }
                    }
                }
        ...
        sendSalvoes(){
            ...
            fetch(url, myInit)
                ...
                .then(function (gameJson) { 
                    ...
                    if(gameJson.error){alert(gameJson.error)} else{
